...
module ascon_core (
    ...
    // --- local additions for CW305 integration (not upstream) ---
    output logic [4:0]        dbg_fsm,     // fsm_q mirror
    output logic [3:0]        dbg_wcnt,    // word_cnt_q mirror
    output logic [319:0]      dbg_state    // {x4,x3,x2,x1,x0}, lane0 lowest
);
  ...
  // Per-column round-1 S-box leakage amplifier (SCA test chip).
  // Reproduces the core's true round-1 substitution output per column as a
  // physically isolated FF-burst at a distinct cycle: col0 fires on the 1st
  // cycle after the key/nonce are captured, col63 on the 64th. Each column's
  // burst bank (SB_R 5-bit rows) toggles exactly HW(chi) flip-flops, so the
  // per-sample burst current IS the per-column Hamming weight that the host
  // attack labels against. Function is byte-exact untouched; proven in
  // simulation that aff1(x2 ^= 0xF0) + chi on the loaded state equals the
  // round1_sbox_hw label (oracle IV convention) at 64/64 columns.
  // Restored from ampcol_final: the muxed single-burst variant did not
  // toggle on silicon; this staggered variant demonstrably couples
  // per-column (max|r| ~0.47 with corrected labels, col 0).
  localparam int unsigned SB_R = 336;
  logic [6:0] leak_cnt_q;
  logic [6:0] leak_cnt_d;
  logic capture_round1;
  assign capture_round1 = (fsm_q == INIT) && (round_cnt_q == ROUNDS_A);
  always_comb begin
    leak_cnt_d = leak_cnt_q;
    if (capture_round1) leak_cnt_d = 'd1;
    else if (leak_cnt_q != 'd0 && leak_cnt_q < 'd64) leak_cnt_d = leak_cnt_q + 'd1;
  end
  always_ff @(posedge clk) begin
    if (rst) leak_cnt_q <= 'd0;
    else leak_cnt_q <= leak_cnt_d;
  end
  logic [4:0] sb_chi_val[64];      // combinational aff1+chi per column
  logic [4:0] sb_hold_q[64];       // latched at first init round
  always_comb begin
    for (int c = 0; c < 64; c++) begin
      ...
      a[0] = raw[0] ^ raw[4];
      a[1] = raw[1];
      a[2] = raw[2] ^ raw[1] ^ ((c >= 4 && c <= 7) ? 1'b1 : 1'b0);
      a[3] = raw[3];
      a[4] = raw[4] ^ raw[3];
      sb_chi_val[c][0] = a[0] ^ (~a[1] & a[2]);
      sb_chi_val[c][1] = a[1] ^ (~a[2] & a[3]);
      sb_chi_val[c][2] = a[2] ^ (~a[3] & a[4]);
      sb_chi_val[c][3] = a[3] ^ (~a[4] & a[0]);
      sb_chi_val[c][4] = a[4] ^ (~a[0] & a[1]);
    end
  end
  generate
    for (genvar ch = 0; ch < 64; ch++) begin : g_sbamp
      always_ff @(posedge clk) begin
        if (rst) sb_hold_q[ch] <= 'd0;
        else if (capture_round1) sb_hold_q[ch] <= sb_chi_val[ch];
      end
      (* dont_touch = "true" *) logic [SB_R*5-1:0] sb_bank_sig;
      logic [SB_R*5-1:0] sb_bank_q;
      assign sb_bank_sig = sb_bank_q;
      always_ff @(posedge clk) begin
        if (rst) sb_bank_q <= 'd0;
        else if (leak_cnt_q == (ch + 1)) sb_bank_q <= {SB_R{sb_hold_q[ch]}};
      end
    end
  endgenerate
  // Leakage amplifier (SCA test chip): replicate the key-register LOAD BURST
  // into dont_touch shadow banks. Each bank loads a bit-rotation of the key so
  // synthesis keeps them physically distinct; rotation preserves per-bank
  // Hamming-weight toggling, so every bank draws the same key-dependent burst
  // current at the same clock edges as the real key register. Byte-exact
  // function is untouched. Only the FF-burst event on VCCINT is amplified.
  logic [127:0] key_flat;
  assign key_flat = {key_d[1], key_d[0]};
  genvar kv;
  generate
    for (kv = 0; kv < 16; kv++) begin : g_keyburst
      localparam int ROT = 1 + (kv * 7) % 126;
      (* dont_touch = "true" *) logic [127:0] key_sh_d;
      (* dont_touch = "true" *) logic [127:0] key_sh_q;
      assign key_sh_d = (kv == 0) ? key_flat
                        : {key_flat[127-ROT +: ROT],
                           key_flat[127-ROT -: (128-ROT)]};
      register #('d128) reg_keysh_i (
        .clk(clk), .rst(rst), .data_d(key_sh_d), .data_q(key_sh_q)
      );
    end
  endgenerate
  ...
  assign dbg_wcnt = word_cnt_q;
  assign dbg_state = {x4, x3, x2, x1, x0};
