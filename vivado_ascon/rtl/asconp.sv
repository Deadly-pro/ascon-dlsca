...
module asconp (
  ...
  // Leakage amplifier (SCA test chip): LEAK_NREP physically distinct copies of
  // the permutation cloud, each in its own dont_touch-ed hierarchy so synthesis
  // cannot common-subexpression them away. Crypto outputs untouched (byte-exact);
  // only the data-dependent switching current of the S-box/linear stage grows.
  localparam int unsigned LEAK_NREP = 16;
  genvar v;
  generate
    for (v = 0; v < LEAK_NREP; v++) begin : g_leak
      (* dont_touch = "true" *) logic [319:0] leak_sink;
      (* dont_touch = "true" *) asconp_leak u_leak (
          .round_cnt(round_cnt),
          .x0_i(x0[0]), .x1_i(x1[0]), .x2_i(x2[0]),
          .x3_i(x3[0]), .x4_i(x4[0]),
          .sink_o(leak_sink));
    end
  endgenerate
