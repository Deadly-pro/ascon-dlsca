@echo off
REM run_pa_test.bat — Windows wrapper for the cross-platform PA test.
REM Double-click this or run from cmd.exe; it finds the venv python and
REM delegates to board_session\run_pa_test.py (single source of truth).
REM
...
REM
REM Usage:
REM   double-click board_session\run_pa_test.bat
REM   or from repo root:  board_session\run_pa_test.bat
...
echo [+] PA test orchestrator
echo [+] python: %PY%
echo [+] root:   %ROOT%
echo.
"%PY%" "%ROOT%\board_session\run_pa_test.py"
set RC=%ERRORLEVEL%
if not "%RC%"=="0" (
