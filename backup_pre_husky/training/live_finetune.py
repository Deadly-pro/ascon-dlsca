...
def main():
    ...
    ap.add_argument('--online', action='store_true',
                    help='interleave model input selection and online gradient updates per trace')
    ap.add_argument('--active-mode', choices=['random', 'separating', 'entropy'], default='random',
                    help='nonce input selection strategy (random, separating, entropy)')
    args = ap.parse_args()
    ...
    if args.online:
        ...
        model.train()
        opt_online = torch.optim.Adam(model.parameters(), lr=args.lr)
        lossf_online = nn.CrossEntropyLoss()

    from adaptive import pick_separating_nonce
    ...
    while len(traces) < args.ntrain:
        ...
        # Output next input (nonce selection strategy)
        if args.active_mode == 'separating':
            nonce = pick_separating_nonce(col, prof.support, rng)
        elif args.active_mode == 'entropy':
            cand_nonces = [os.urandom(16) for _ in range(16)]
            best_entropy, best_n = -1.0, cand_nonces[0]
            n_classes = len(prof.classes)
            for cn in cand_nonces:
                pred_classes = lab.hypothesis_labels(col, np.frombuffer(cn, np.uint8)[None], hyps)[0]
                c_counts = np.bincount(pred_classes, minlength=n_classes)
                probs = c_counts / max(1, c_counts.sum())
                ent = -float(np.sum([p * np.log(p + 1e-12) for p in probs if p > 0]))
                if ent > best_entropy:
                    best_entropy, best_n = ent, cn
            nonce = best_n
        else:
            nonce = os.urandom(16)
        ...
        pred = lab.hypothesis_labels(col, np.frombuffer(nonce, np.uint8)[None],
                                     hyps)[0]
        lbl = int(pred[hyp_true])
        traces.append(trace)
        labels.append(lbl)

        # Online step: immediate gradient update on newly acquired trace
        if args.online:
            model.train()
            X_curr = np.array([trace])
            Xf_curr = build_input(X_curr, prof.arch, prof.window, model)
            Xt_c = torch.as_tensor(Xf_curr, dtype=torch.float32)
            Yt_c = torch.tensor([lbl], dtype=torch.int64)
            opt_online.zero_grad()
            loss_c = lossf_online(model(Xt_c), Yt_c)
            loss_c.backward()
            opt_online.step()

        if len(traces) % 50 == 0:
            print(f'    captured & trained {len(traces)}/{args.ntrain} '
                  f'[{time.time()-t0:.0f}s]')
    ...
    Xt = torch.as_tensor(Xf_all, dtype=torch.float32)
    Yt = torch.tensor(Y)
    model.train()
    n_classes = len(prof.classes)
    counts = np.bincount(Y, minlength=n_classes)
    safe_counts = np.where(counts == 0, 1.0, counts.astype(np.float64))
    w = torch.tensor(len(Y) / (n_classes * safe_counts), dtype=torch.float32)
    w[counts == 0] = 0.0
