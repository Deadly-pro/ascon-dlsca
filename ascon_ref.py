...
def read_data_from_file(filename="data.txt"):
    ...
    # Interpreta come esadecimale (senza 0x) per lunghezze
    aad_len = int(lines[0].split()[1], 16)
    aad_data = bytes.fromhex(lines[1].strip())

    msg_len = int(lines[2].split()[1], 16)
    msg_data = bytes.fromhex(lines[3].strip())

    assert len(aad_data) == aad_len, f"AAD length mismatch: expected {aad_len}, got {len(aad_data)}"
    assert len(msg_data) == msg_len, f"MSG length mismatch: expected {msg_len}, got {len(msg_data)}"

    #print(f"AAD (len {aad_len}):       {aad_data.hex()}")
    #print(f"Plaintext (len {msg_len}): {msg_data.hex()}")

    return aad_data, msg_data
...
def ascon_process_associated_data(S, b, rate, associateddata):
    ...
    if len(associateddata) > 0:
        ...
        for block in range(0, len(a_padded), rate):
            ...
            #print(f"  AAD block x0 = {aad_block0:016x}")
            S[0] ^= aad_block0

            if rate == 16:
                aad_block1 = bytes_to_int(a_padded[block+8:block+16])
                #print(f"  AAD block x1 = {aad_block1:016x}")
                S[1] ^= aad_block1
...
def batch_fpga_expected(pairs, return_mask=False):
    result = [fpga_expected(k, n) for k, n in pairs]
    if return_mask:
